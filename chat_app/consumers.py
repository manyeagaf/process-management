...
# ).get_or_create(name=self.conversation_name)
    ...
    def get_receiver(self):
        emails = self.conversation_name.split("__")
        for email in emails:
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        if message_type == "image":
            ...
            message = Message.objects.create(from_user = self.user,to_user = self.get_receiver(),content =content["message"],message_type = content['type'],conversation = self.conversation)
...
class NotificationConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        conversations = Conversation.objects.filter(name__contains = self.user.email)
